...
void loop() {
    ...
    if (UltraSonic()) {
        // Move backward and handle obstacle avoidance
        backward();
        delay(220);
        stopMotors();
        delay(500);
...
void blinkLight() {
    ...
    delay(1000);  // Light stays on for 500ms
    digitalWrite(A1, LOW);
    delay(1000);  // Light stays off for 500ms
}
...
bool compareArrays(float arr1[], float arr2[]) {
    // Iterate through all pairs of elements to check for a match
